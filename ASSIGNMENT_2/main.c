...
#define NXP_KEIL_VID 0x0930
#define NXP_MSD_PID  0x6544

#define VID_PD_2 0x0781
#define PID_PD_2 0x558A
...
static int test_mass_storage(struct usb_device *device, uint8_t endpoint_in, uint8_t endpoint_out)
{
	...
	int read_cnt;
	   ...
	   printk("Reseting mass storage device");
...
static int usbdev_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
	...
	printk(KERN_INFO "INTERFACE SUB CLASS : %x", interface->cur_altsetting->desc.bInterfaceSubClass);
	printk(KERN_INFO "INTERFACE PROTOCOL : %x", interface->cur_altsetting->desc.bInterfaceProtocol);
	...
	for(i=0;i<interface->cur_altsetting->desc.bNumEndpoints;i++)
	{
	...
	
		{
			if(epAddr & 0x80)
				{
				endpoint_in = epAddr;
				printk(KERN_INFO "EP No.= %d and Type of Endpoint is Bulk IN\n", i);
				
				}
			else
				{
				endpoint_out = epAddr;
				printk(KERN_INFO "EP No.= %d and Type of Endpoint is Bulk OUT\n", i);
